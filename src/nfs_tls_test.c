...
static void *worker_thread(void *arg)
{
	...
	while (!atomic_load_explicit(&g_stop, memory_order_relaxed) &&
		...
		if (o->o_rate > 0) {
			...
			w->w_next_conn_time +=
				(double)o->o_threads / (double)o->o_rate;
		}

		atomic_fetch_add_explicit(&w->w_attempts, 1,
		...
		int rc = tls_connect_starttls(
			o->o_host, o->o_port, w->w_ctx,
			o->o_no_session_reuse ? NULL : w->w_session, &conn, xid,
			&timing, errbuf, sizeof(errbuf));
		if (rc < 0) {
			/*
			...
			if (strncmp(errbuf, "connect(", 8) == 0 ||
				...
				atomic_fetch_add_explicit(&w->w_fail_handshake,
							  1,
							  memory_order_relaxed);
			}
		...
		if (o->o_tls_info) {
			...
			if (atomic_compare_exchange_strong_explicit(
				...
				char alpn[64];
				if (tls_conn_get_alpn(&conn, alpn,
						      sizeof(alpn)) > 0)
					printf("ALPN: %s\n", alpn);
			}
		}
		...
		if (o->o_check_san) {
			if (tls_conn_check_san(&conn, o->o_check_san, errbuf,
					       sizeof(errbuf)) < 0) {
				atomic_fetch_add_explicit(&w->w_fail_handshake,
							  1,
							  memory_order_relaxed);
		...
		if (o->o_require_tls13) {
			...
			if (strcmp(ver, "TLSv1.3") != 0) {
				snprintf(
					...
					"TLS version %s below required TLSv1.3",
					ver);
				atomic_fetch_add_explicit(&w->w_fail_handshake,
							  1,
							  memory_order_relaxed);
		...
		w->w_session = o->o_no_session_reuse ?
				       NULL :
		...
		for (int c = 0; c < calls && rpc_ok; c++) {
			...
			double r0 = now_ms();
			int rrc = tls_send_nfs_null(&conn, rxid, errbuf,
						    sizeof(errbuf));
			double r1 = now_ms();
			if (rrc < 0) {
				rpc_ok = 0;
		...
		if (rpc_ok) {
			...
			s.s_resumed = resumed;
			sa_add(&w->w_sa, &s);
			atomic_fetch_add_explicit(&w->w_ok, 1,
						  memory_order_relaxed);
...
static void *progress_thread(void *arg)
{
	...
	while (!atomic_load_explicit(&g_progress_stop, memory_order_relaxed)) {
		...
		if (atomic_load_explicit(&g_progress_stop,
					 memory_order_relaxed))
			break;
		...
		for (int i = 0; i < ps->nworkers; i++) {
			total_att +=
				atomic_load_explicit(&ps->workers[i].w_attempts,
						     memory_order_relaxed);
			total_ok += atomic_load_explicit(&ps->workers[i].w_ok,
							 memory_order_relaxed);
			long ftcp =
				atomic_load_explicit(&ps->workers[i].w_fail_tcp,
						     memory_order_relaxed);
			long fprb = atomic_load_explicit(
				&ps->workers[i].w_fail_probe,
				memory_order_relaxed);
			long fhs = atomic_load_explicit(
				&ps->workers[i].w_fail_handshake,
				memory_order_relaxed);
			long frpc =
				atomic_load_explicit(&ps->workers[i].w_fail_rpc,
						     memory_order_relaxed);
...
static void parse_options(int argc, char **argv, struct options *o)
{
	...
	while ((ch = getopt_long(argc, argv, "", long_opts, NULL)) != -1) {
		...
		case 't':
			...
			break;
		case 'd':
			o->o_duration = atol(optarg);
			break;
		case 'K':
			o->o_calls_per_conn = atoi(optarg);
			break;
		case 'R':
			o->o_no_session_reuse = 1;
			break;
		case 'r':
			o->o_rate = atol(optarg);
			break;
		case 'P':
			o->o_progress = atoi(optarg);
			break;
		case 'I':
			o->o_tls_info = 1;
			break;
		...
		case 'L':
			...
			break;
		case 'S':
			o->o_check_san = optarg;
			break;
		case 'T':
			o->o_snapshot_stats = 1;
			break;
		case 'D':
			o->o_diagnose = 1;
			break;
...
static void print_report(const struct options *o, struct worker *workers,
			 int nworkers, double elapsed_ms)
{
	...
	printf("\nnfs_tls_test: %s:%s, %d thread%s", o->o_host, o->o_port,
	...
	if (!o->o_no_null)
		printf(", %d call%s/conn", o->o_calls_per_conn,
		       o->o_calls_per_conn > 1 ? "s" : "");
	printf("\n");
	...
	if (o->o_tls_info &&
	    !atomic_load_explicit(&g_tls_info_printed, memory_order_relaxed))
			   ...
			   int last)
{
	printf("    \"%s\": { \"p50\": %.3f, \"p95\": %.3f, "
	       ...
	       name, r->p50, r->p95, r->p99, r->p999, r->avg, r->min, r->max,
	       last ? "" : ",");
}

static void print_report_json(const struct options *o, struct worker *workers,
			      int nworkers, double elapsed_ms)
{
	size_t total = 0;
	long total_ok = 0, fail_tcp = 0, fail_probe = 0;
...
int main(int argc, char **argv)
{
	...
	if (o.o_duration > 0) {
		...
		iter_per_worker =
			(size_t)o.o_duration * max_rate / (size_t)nworkers +
			1024;
	} else {
		iter_per_worker =
	...
	for (int i = 0; i < nworkers; i++) {
		...
		if (sa_init(&workers[i].w_sa, iter_per_worker) < 0) {
			fprintf(stderr,
				"Out of memory allocating sample array "
				"(worker %d, %zu samples)\n",
				i, iter_per_worker);
			for (int j = 0; j < i; j++)
	...
	for (int i = 0; i < nworkers; i++) {
		if (pthread_create(&workers[i].w_thread, NULL, worker_thread,
				   &workers[i]) != 0) {
	...
	if (o.o_progress > 0) {
		if (pthread_create(&progress_tid, NULL, progress_thread, &ps) !=
		    0) {
			fprintf(stderr,
				"pthread_create progress thread failed\n");
			progress_tid = 0;
		}
	}
	...
	if (o.o_duration > 0) {
		clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &duration_end,
				NULL);
		atomic_store_explicit(&g_stop, 1, memory_order_release);
	}
	...
	if (progress_tid) {
		atomic_store_explicit(&g_progress_stop, 1,
	...
	for (int i = 0; i < nworkers; i++) {
		...
		fail_probe_total += atomic_load_explicit(
			&workers[i].w_fail_probe, memory_order_relaxed);
		fail_hs_total += atomic_load_explicit(
			&workers[i].w_fail_handshake, memory_order_relaxed);
		fail_rpc_total += atomic_load_explicit(&workers[i].w_fail_rpc,
						       memory_order_relaxed);
	}
	long total_fail = fail_tcp_total + fail_probe_total + fail_hs_total +
			  fail_rpc_total;
