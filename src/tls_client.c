 ...
 */
static int tcp_connect(const char *host, const char *port, char *errbuf,
		       size_t errsz)
{
 ...
 */
static int send_auth_tls_probe(int fd, uint32_t xid, char *errbuf, size_t errsz)
{
	uint8_t buf[64];
	size_t len = rpc_build_null_call(buf, sizeof(buf), xid, NFS_PROGRAM,
					 NFS_VERSION_4, RPC_AUTH_TLS);
	if (len == 0) {
		snprintf(errbuf, errsz,
			 "rpc_build_null_call: buffer too small");
		return -1;
	}
	if (rpc_writen(fd, buf, len) != (ssize_t)len) {
		snprintf(errbuf, errsz, "write AUTH_TLS probe: %s",
			 strerror(errno));
		return -1;
	}
	return 0;
 ...
 * Returns 0 on success, -1 on error (errbuf filled).
 */
static int recv_starttls_reply(int fd, uint32_t expected_xid, char *errbuf,
			       size_t errsz)
{
	...
	if (body_len == 0 || body_len > STARTTLS_REPLY_MAX) {
		snprintf(errbuf, errsz,
			 "STARTTLS reply: implausible body length %u",
			 body_len);
		return -1;
	}
	...
	if (xid != expected_xid) {
		snprintf(errbuf, errsz,
			 "STARTTLS reply: xid mismatch (got %u, want %u)", xid,
			 expected_xid);
		return -1;
	}
	if (msg_type != RPC_REPLY) {
		snprintf(errbuf, errsz,
			 "STARTTLS reply: expected REPLY (1), got %u",
			 msg_type);
		return -1;
	}
	...
	if (!rpc_get_u32(body, body_len, &pos, &verf_flavor) ||
		...
		snprintf(errbuf, errsz,
			 "STARTTLS reply: short verifier header");
		return -1;
	}
	...
	if (accept_stat != RPC_SUCCESS) {
		snprintf(errbuf, errsz,
			 "STARTTLS reply: accept_stat %u (not SUCCESS)",
			 accept_stat);
		return -1;
	}
	return 0;
			 ...
			 size_t errsz)
{
	...
	if (SSL_set_alpn_protos(conn->tc_ssl, alpn_sunrpc,
				sizeof(alpn_sunrpc))) {
		      ...
		      size_t errsz)
{
	...
	if (!(marker & RPC_LAST_FRAG)) {
		snprintf(errbuf, errsz,
			 "NULL reply: multi-fragment reply unexpected");
		return -1;
	}
	uint32_t body_len = marker & ~RPC_LAST_FRAG;
	if (body_len == 0 || body_len > STARTTLS_REPLY_MAX) {
		snprintf(errbuf, errsz,
			 "NULL reply: implausible body length %u", body_len);
		return -1;
	}
	...
	if (rxid != xid) {
		snprintf(errbuf, errsz,
			 "NULL reply: xid mismatch (got %u, want %u)", rxid,
			 xid);
		return -1;
	}
			 ...
			 const char *value)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if ((type_hint == 0 || type_hint == GEN_IPADD) &&
			...
			const char *p = NULL;
			if (iplen == 4)
				p = inet_ntop(AF_INET, ipdata, ipstr,
					      sizeof(ipstr));
			else if (iplen == 16)
				p = inet_ntop(AF_INET6, ipdata, ipstr,
					      sizeof(ipstr));
		       ...
		       char *errbuf, size_t errsz)
{
	...
	if (!sans) {
		snprintf(errbuf, errsz,
			 "peer certificate has no subjectAltName");
		X509_free(cert);
		return -1;
	}
