...
static size_t build_null_reply(uint8_t *buf, size_t bufsz, uint32_t xid)
{
	...
	if (!rpc_put_u32(buf, bufsz, &pos, xid))
		return 0;
	if (!rpc_put_u32(buf, bufsz, &pos, RPC_REPLY))
		return 0;
	if (!rpc_put_u32(buf, bufsz, &pos, RPC_MSG_ACCEPTED))
		return 0;
	if (!rpc_put_u32(buf, bufsz, &pos, RPC_AUTH_NONE))
		return 0;
	if (!rpc_put_u32(buf, bufsz, &pos, 0u))
		return 0;
	if (!rpc_put_u32(buf, bufsz, &pos, RPC_SUCCESS))
		return 0;
	return pos; /* 28 */
}
			       ...
			       uint32_t *body_len_out)
{
	*body_len_out = 0;
	...
	while (got < body_len) {
		...
		if (r == 0)
			return -1; /* EOF mid-record is an error */
		if (r < 0)
			return -1;
		got += (size_t)r;
	}
			  ...
			  uint32_t *xid_out, uint32_t *cred_flavor_out)
{
	...
	if (!rpc_skip(body_len, &pos, cred_padded))
		return 0;
